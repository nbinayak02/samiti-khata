import {
  Dialog,
  DialogClose,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { useState } from "react";
import { PlusCircle } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Controller, useForm } from "react-hook-form";
import { Separator } from "@/components/ui/separator";
import { zodResolver } from "@hookform/resolvers/zod";
import { Field, FieldError, FieldLabel } from "@/components/ui/field";
import { fiscalYearSchema } from "../schemas/fiscal-year.schema";
import useCreateFiscalYear from "../hooks/useCreateFiscalYear";
import { SubmitButton } from "@/components/shared/form";

export default function CreateFiscalYearDialog() {
  const [open, setOpen] = useState<boolean>(false);

  const form = useForm({
    resolver: zodResolver(fiscalYearSchema),
    defaultValues: {
      endDateBs: "",
      startDateBs: "",
    },
  });

  const { isPending, onCreate } = useCreateFiscalYear({
    setDialogClose: setOpen,
    resetForm: form.reset,
  });
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        render={
          <Button>
            <PlusCircle /> Create Fiscal Year
          </Button>
        }
      />
      <DialogContent className="w-full min-w-md flex flex-col">
        <DialogHeader>
          <DialogTitle>Create Fiscal Year</DialogTitle>
          <DialogDescription>
            Enter year details. Click create when you are done.
          </DialogDescription>
        </DialogHeader>
        <form className="grid gap-3" onSubmit={form.handleSubmit(onCreate)}>
          <Controller
            name="startDateBs"
            control={form.control}
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel htmlFor="name">
                  Start Date (B.S.)
                  <span className="text-rose-500">*</span>
                </FieldLabel>
                <Input
                  {...field}
                  id="name"
                  aria-invalid={fieldState.invalid}
                  placeholder="2083-04-01"
                />
                {fieldState.invalid && (
                  <FieldError errors={[fieldState.error]} />
                )}
              </Field>
            )}
          />

          <Controller
            name="endDateBs"
            control={form.control}
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel htmlFor="name">
                  End Date (B.S.)
                  <span className="text-rose-500">*</span>
                </FieldLabel>
                <Input
                  {...field}
                  id="name"
                  aria-invalid={fieldState.invalid}
                  placeholder="2084-03-30"
                />
                {fieldState.invalid && (
                  <FieldError errors={[fieldState.error]} />
                )}
              </Field>
            )}
          />
          <Separator />
          <DialogFooter>
            <DialogClose render={<Button variant="outline">Cancel</Button>} />
            <SubmitButton
              isPending={isPending}
              label="Create Fiscal Year"
              labelWhenPending="Creating"
            />
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
}
